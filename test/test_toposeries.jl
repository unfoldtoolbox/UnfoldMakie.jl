...
@testset "error checking: bin_width and bin_num specified" begin
    ...
    @test err1 == ErrorException("Ambigious parameters: specify only `bin_width` or `bin_num`.")
end
...
@testset "error checking: bin_width and bin_num not specified" begin
    ...
    @test err1 == ErrorException("You haven't specified `bin_width` or `bin_num`. Such option is available only with categorical `mapping.col` or `mapping.row`.")
end
...
@testset "toposeries with adjusted ylim_topo" begin
    plot_topoplotseries(df; bin_width, positions = positions, axis = (; ylim_topo = (0, 0.7)))
end
