...
include("overrider_bivariate.jl")
include("overrider_contours.jl")
# -----------------------------------------------------------------------------
...
# Register a live Observable as a real plot attribute so `tp.attr[]` works.
function ensure_live_plot_attribute!(p::TopoPlot, key::Symbol, obs::Observable)
    haskey(p.attributes, key) || ComputePipeline.add_input!(p.attributes, key, obs)
    return obs
end
...
function setup_positions_and_geometry(p)
    ...
    ensure_live_plot_attribute!(p, :xg, xg)
    ensure_live_plot_attribute!(p, :yg, yg)
...
function occupied_bivariate_bins(UVnorm, mask, opts)
    cb = opts.colorbox
    cb === nothing && error("Bivariate occupancy needs a colorbox.")

    lift(UVnorm, mask) do UVn, mask_
        U01, V01 = UVn[1], UVn[2]
        n_rows, n_cols = size(cb)
        occupied = falses(n_rows, n_cols)
        ...
        @inbounds for i in axes(U01, 1), j in axes(U01, 2)
            ...
            # Keep occupancy consistent with the discrete `:nearest` sampling
            # used by the corner/range bivariate topoplots.
            xi = clamp(round(Int, 1 + u * (n_cols - 1)), 1, n_cols)
            yi = clamp(round(Int, 1 + v * (n_rows - 1)), 1, n_rows)
            occupied[yi, xi] = true
        end
        occupied
    end
...
function draw_bivariate_path!(p, xg, yg, geometry, padded_pos_data_bb, mask)
    ...
    ensure_live_plot_attribute!(p, :bivariate_occupied_bins, occupied_bivariate_bins(UVnorm, mask, opts))
...
function draw_scalar_path!(p, xg, yg, padded_pos_data_bb, mask, colorrange)
    ...
    ensure_live_plot_attribute!(p, :data_interpolated, data)
...
function Makie.plot!(p::TopoPlot)
    ...
    if p.interpolation[] isa DelaunayMesh
        ...
        ensure_live_plot_attribute!(p, :mask, mask)
