...
function load_erp(subj, task)
    local_name = joinpath("/scratch/users/mikheev/erpcore", "erpcore_$(subj)_$(task).erp")
    isfile(local_name) && return MAT.matread(local_name)["ERP"]
...
function load_erp_subject_or_nothing(
    task;
    subject,
    timepoint=92,
    condition=1,
    ...
    try
        ...
        if occursin("does not exist", msg)
            println("skipped: erpcore_$(subject)_$(task).erp")
            return nothing
        end
        rethrow()
    end
...
task_configs_30 = [
    (task = "MMN", subjects = [1, 2, 9, 10, 11, 15, 16, 20, 25, 26], timepoint = 96, condition = 3),
    (task = "N170", subjects = [5, 6, 7, 12, 15, 16, 18, 24, 26, 27], timepoint = 105, condition = 2),
    (task = "P3", subjects = [1, 2, 4, 7, 8, 10, 12, 13, 16, 19], timepoint = 129, condition = 1),
]

task_configs_30_balanced = [
    (task = "MMN", subjects = [1, 2, 11, 15, 16, 19, 20, 26, 29, 30], timepoint = 96, condition = 3),
    (task = "N170", subjects = [6, 7, 8, 10, 12, 15, 16, 24, 26, 27], timepoint = 105, condition = 2),
    (task = "P3", subjects = [4, 7, 8, 10, 12, 13, 16, 19, 20, 24], timepoint = 129, condition = 1),
]


task_configs_all = [
    (task = "MMN", subjects = 1:30, timepoint = 96, condition = 3),
    (task = "N170", subjects = 1:30, timepoint = 105, condition = 2),
    (task = "P3", subjects = 1:30, timepoint = 129, condition = 1),
]
