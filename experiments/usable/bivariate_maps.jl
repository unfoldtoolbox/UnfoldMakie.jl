...
function nice_ticks(x; n=5, digits=2, upper_lim = 5)
    lo, hi = if x isa AbstractVector
        UnfoldMakie._topo_range_from_values(x)
    elseif x isa Tuple && length(x) == 2
        x
    else
        error("nice_ticks expects a vector of values or a (lo, hi) tuple.")
    end
    t = collect(range(lo, hi; length=n))
    t2 = collect(range(0, upper_lim; length=n))
    labels = [
        isapprox(v, 0; atol=10.0^(-digits)) ? "0" :
        string(round(v; digits=digits))
        for v in t
    ]
    return (t2, labels)
end
